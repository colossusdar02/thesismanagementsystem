...
$query = mysql_query("SELECT * FROM viewcounter WHERE id = 'COLAW'");
while($row = mysql_fetch_array($query)){
$date = $row["date"];
$id = $row["id"];
$views = $row["views"];
};
if($date == $dateToday && $id == 'COLAW'){
	$view = $views + 1;
	mysql_query("UPDATE viewcounter SET `views` = '$view' WHERE id='COLAW'");
}
else{
	$view = 1;
	mysql_query("INSERT INTO viewcounter (id, views, date) VALUES ('COLAW', '$view', '$dateToday')");
}
?>
<?php
include('inc/db.php');
...
<body>
	<table id="tableCollege">
		<tr>
					...
					<form action="search_colaw.php" method="GET">
					...
					<td>&nbsp;</td>
					<td><button type="button" name="submit" class="btn btn-success" onclick="location.href='/sysiac1/codes_user/colaw.php'"><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span>&nbsp;View All</button></td>
					<td><a href="/sysiac1/codes_user/colleges.php"><img src="/sysiac1/images/colaw.png" style="width: 80px; margin-left: 249px;"></a></td>
		</tr>
	</table>
	<?php 
	...
	thesistitle LIKE '%".$search."%' AND thesiscollege = 'colaw' || thesisauthors LIKE '%".$search."%' AND thesiscollege = 'colaw' || thesisschoolyear LIKE '%".$search."%' AND thesiscollege = 'colaw' || thesiscourse LIKE '%".$search."%' AND thesiscollege = 'colaw' || thesisdateuploaded LIKE '%".$search."%' AND thesiscollege = 'colaw'
